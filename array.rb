class Array
  ...
  # e.g. [[1,2], [3,4], [4,5,6,7,8,9], [3,2], [3,5]].even_partition_by_size(3){|a| a.select(&:even?)}
  def even_partition_by_size(num_partitions)
    total_deep_size = self.map{|a| block_given? ? yield(a) : a.size }.inject(&:+)
    ...
    self.each do |array|
      size_of_array = block_given? ? yield(array) : array.size
